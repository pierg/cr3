...
from goals.shared.paths import output_folder_cgg
from goals.tools.strings import tab, tabar
from logic.tools.crome_io import save_to_file
...
class Cgg:
    ...
    def _build_graph(self) -> None:
        from goals.cgg.context_based_clustering import context_based_goal_clustering
    ...
    def add_node(self, goal: Goal):
        if self.get_goal_by_id(goal_id=goal.id) is None:
            self._graph.add_node(goal, name=goal.id, goal=goal)
    ...
    def draw(self):
        ...
        if self.n_nodes > 0:
            ...
            plt.savefig(output_folder_cgg / "crome.pdf")
