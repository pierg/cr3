...
from goals.shared.paths import persistence_path
...
def dump_cgg(cgg: Cgg, folder_name: str = ""):
    ...
    with open(persistence_path / folder_name / "crome.dat", "wb") as stream:
        dill.dump(cgg, stream)


def load_cgg(folder_name: str = "") -> Cgg | None:
    if not os.path.exists(persistence_path / folder_name / "crome.dat"):
        return None

    with open(persistence_path / folder_name / "crome.dat", "rb") as stream:
