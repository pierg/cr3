...
from logic.tools.string_manipulation import pyeda_syntax_fix
from logic.typelement.basic import Boolean

from synthesis.atom import Atoms, AtomValues
...
def extract_transitions(
    ...
    if "/" in formula:
        insouts = formula.split("/")
        formula = f"({insouts[0]}) -> ({insouts[1]})"

    if pyeda_syntax_fix(formula) == "1":
        return {(Atoms.any(), Atoms.any())}

    print(f"BOOL\t{formula}")
